...
Working AIは`../working-ai/AGENT.md`および`../working-ai/$SAMPLE_WORK/BLUEPRINT.md`に基づいて`working-ai/$SAMPLE_WORK/artifact/*`を作成します。

Checking AIはWorking AIが正しく作成できているか以下の観点で評価し、必要に応じて`../working-ai/AGENT.md`やこの評価指針を更新してください。ただし、AGENT.mdと評価指針はあらゆるWorking AIのartifactに共通であるため、汎用的であることが求められます。
...
2. 変更があったworking aiのBLUEPRINTと`../working-ai/AGENT.md`に書かれたゴール・Deliverableを把握する。
3. Artifactの内容とREADMEの手順が一致しているかを確認する。
4. 検証手順を実際に実行できるか(理論上)をチェックし、欠けているものがあれば指摘する。
5. 評価結果を反映したフィードバックを`../working-ai/AGENT.md`に記録し、このファイルの評価項目も最新化する。
